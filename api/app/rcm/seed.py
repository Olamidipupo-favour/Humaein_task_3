...
class RCMSeeder:
    ...
    def seed_encounters(self, providers: List[Provider], patients: List[Patient]) -> List[Encounter]:
        """Seed encounter data."""
    ...
    def seed_claims(self, encounters: List[Encounter], payers: List[Payer]) -> List[Claim]:
        """Seed claim data."""
        claims = []
    ...
    def seed_remittances(self, claims: List[Claim], payers: List[Payer]) -> List[Remittance]:
        ...
        for claim in claims:
            if claim.status in [ClaimStatus.PAID, ClaimStatus.PROCESSING]:
                remittance = Remittance(
                    ...
                    payment_date=claim.submission_date + timedelta(days=random.randint(15, 45)),
                    ...
                    check_number=f"CHK{random.randint(100000, 999999)}" if random.choice([True, False]) else None,
                    claim_id=claim.id,
        ...
        for claim in claims:
            if claim.status == ClaimStatus.DENIED:
                denial = Denial(
                    ...
                    denial_date=claim.submission_date + timedelta(days=random.randint(10, 30)),
                    ...
                    appeal_deadline=datetime.now() + timedelta(days=random.randint(30, 90)),
